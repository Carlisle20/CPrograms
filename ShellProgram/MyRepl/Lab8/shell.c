 ...
 * shell.c
 * Carlisle Calabrese
 * This is the shell program that I developed as part of my Operating Systems Class. 
 *It has most of the basic features, including a basic yet also pretty robust job handler. 
 *Additonally, since its completion, many advanced features were also added on as well, 
 *such as a fully functioning restart system to restart halted jobs and processes.
 *Code Created (Originally): March 30, 2021.
 *Code Modified: July 26, 2021.
 */

/*
 * Get csapp.h and csapp.c from http://csapp.cs.smu.edu/3e/code.html
 *
 * IMPORTANT: In order to compile the shell, you must use cd myrepl/Lab8 then compile everything using the command below.
 *
